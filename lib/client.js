...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function termBody(args, cv, rv, running, block) {
    ...
    function renderDetail(o) {
      ...
      var cv = o.cv;
      var kids = [];
      if (rv && rv.card === "terminal") kids.push(termBody(o.args, cv, rv, o.running, o.block));
      ...
      else if (cv && cv.card === "terminal") kids.push(termBody(o.args, cv, null, o.running, o.block));
    ...
    function ToolRow(props) {
      ...
      var bodyKids = renderDetail({ name: wireName, block: block, running: running, args: args, argsRaw: argsRaw, rv: rv, cv: cv, cwd: props.cwd });
...
.hbt-term { border-radius: 6px; overflow: hidden; background: #0d1117; color: #d6e0ea; }
.hbt-term-cmd { padding: 6px 10px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 11.5px; color: #7ee787; border-bottom: 1px solid rgba(255,255,255,.08); white-space: pre-wrap; word-break: break-all; }
...
.hbt-dock { box-sizing: border-box; width: 100%; max-width: var(--dsh-chat-content-width); margin: 0 auto; padding: 2px 0 0; }
/* 预览激活时 TurnStatus（Deep diving）让位：预览占据流底部状态位，预览消失后自动恢复（slot 系统不产 data-slot 属性，唯一确定的共同祖先是 body） */
body:has(.hbt-dock) [class*="_turnStatus"] { display: none; }
...
`;
    ...
    // ---------- 流式工具预览（conversation.input.dock） ----------
    // DSH 的 tool-call 节点由 tool/call 事件物化，即参数流完、开始执行才出现；
    // 流式期间参数只在 partial.blocks 的 {kind:'tool-call', name, argsRaw} 里累积。
    // 本组件把那个在途调用渲染成一行运行态预览，物化后自动让位给正式卡片。
    ...
    function StreamingToolDock(props) {
      ...
      if (!tc) return null;
      var name = typeof tc.name === "string" ? tc.name : "";
      var argsRaw = typeof tc.argsRaw === "string" ? tc.argsRaw : "";
      var meta = metaFor(name, {});
      var target = looseArg(argsRaw, "file_path") || looseArg(argsRaw, "path") || looseArg(argsRaw, "command") || looseArg(argsRaw, "pattern") || looseArg(argsRaw, "query") || "";
      if (target.length > 90) target = target.slice(0, 90) + "…";
      return e("div", { className: "hbt-dock" },
        e("div", { className: "hbt-row", "data-state": "running" },
          e("span", { className: "hbt-rail" }),
          e("div", { className: "hbt-card" },
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("tool.call.toolview", function () {
        ...
        for (var i = 0; i < rowKeys.length; i++) {
          disposers.push(ctx.slots.register({ name: "tool.call.toolview", key: rowKeys[i] }, (props) => e(ToolRow, props)));
        }
        for (var j = 0; j < goalKeys.length; j++) {
          disposers.push(ctx.slots.register({ name: "tool.call.toolview", key: goalKeys[j] }, (props) => e(GoalCard, props)));
        }
        return function () {
      ...
      ctx.slots.inject("conversation.input.dock", function () {
        return ctx.slots.register(
          { name: "conversation.input.dock", id: "hbt-streaming-tool", order: 50, label: "流式工具预览" },
