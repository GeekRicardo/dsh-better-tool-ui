...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // sessions 只用于扫出会话里出现过的 MCP 工具名（见 startMcpDiscovery）
    ...
    function ToolRow(props) {
      ...
      var defaultOpen = useAutoOpenHold(running && wireName === "bash", block.callId || "");
      ...
      var bodyKids = renderDetail({ name: wireName, block: block, running: running, args: args, argsRaw: argsRaw, rv: rv, cv: cv, cwd: props.cwd, open: open, mcp: mcp });

      // 子调用（subagent 等）递归成行，左竖线结构
      var subs = Array.isArray(block.subCalls) ? block.subCalls : [];
      if (subs.length > 0 && depth < 2) {
        bodyKids.push(e("div", { className: "btu-subs", key: "subs" }, subs.slice(0, 20).map((sub, j) => {
    ...
    function GoalCard(props) {
      ...
      return e("div", { className: "btu-callout", "data-tone": tone },
        e("div", { className: "btu-callout-head" },
          e("span", { className: "btu-callout-dot" }),
          e("span", { className: "btu-callout-title" }, title)),
        e("div", { className: "btu-callout-body" }, body || "（无内容）"),
        rounds !== null ? e("div", { className: "btu-callout-rounds" }, "最多 " + rounds + " 轮") : null);
    }
...
.btu-subs { margin: 4px 2px 4px 6px; padding-left: 10px; border-left: 2px solid color-mix(in srgb, var(--dsw-alias-brand-primary) 22%, transparent); }
...
`;
    ...
    function streamingRow(name, argsRaw) {
    ...
    // ---------- MCP 工具名的动态认领 ----------
    // keyed 槽位按精确 key 派发，而 MCP 的公开名（mcp__<server>__<tool>）由用户装了哪些
    // 服务器决定，插件里无从枚举。所以从 ctx.sessions 的会话快照里扫出 mcp__ 开头的调用名，
    // 见一个认领一个；认领过的名字落 localStorage，下次启动先行注册，省掉首帧的出厂卡片闪烁。
    var MCP_CACHE_KEY = "dsh-better-tool-ui:mcp-tools";
    var MCP_CACHE_MAX = 400;
    var mcpKnown = new Set();
    var mcpListeners = new Set();

    function loadMcpCache() {
      try {
        var list = JSON.parse(window.localStorage.getItem(MCP_CACHE_KEY) || "[]");
        if (!Array.isArray(list)) return;
        for (var i = 0; i < list.length && mcpKnown.size < MCP_CACHE_MAX; i++) {
          if (typeof list[i] === "string" && list[i].indexOf("mcp__") === 0) mcpKnown.add(list[i]);
        }
      } catch (err) { /* 隐私模式或缓存损坏：当作没有缓存 */ }
    }

    function saveMcpCache() {
      try { window.localStorage.setItem(MCP_CACHE_KEY, JSON.stringify(Array.from(mcpKnown))); } catch (err) { /* 写不进去不影响本次会话 */ }
    }

    function noteMcpName(name) {
      if (mcpKnown.has(name) || mcpKnown.size >= MCP_CACHE_MAX) return;
      mcpKnown.add(name);
      saveMcpCache();
      mcpListeners.forEach((fn) => {
        try { fn(name); } catch (err) { console.error("dsh-better-tool-ui: mcp claim failed", err); }
      });
    }

    // 把已知的和之后新发现的 MCP 名字都注册到一个槽位上；返回的 disposer 撤掉全部注册。
    function claimMcpKeys(register) {
      var disposers = new Map();
      var claim = function (name) {
        if (disposers.has(name)) return;
        try { disposers.set(name, register(name)); } catch (err) { console.error("dsh-better-tool-ui: mcp register failed", err); }
      };
      mcpKnown.forEach(claim);
      mcpListeners.add(claim);
      return function () {
        mcpListeners.delete(claim);
        disposers.forEach((d) => {
          try { if (typeof d === "function") d(); } catch (err) { console.error("dsh-better-tool-ui: mcp unregister failed", err); }
        });
        disposers.clear();
      };
    }

    function startMcpDiscovery(ctx) {
      ...
      function walk(blocks, depth) {
        for (var i = 0; i < blocks.length; i++) {
          ...
          if (typeof n === "string" && n.indexOf("mcp__") === 0) noteMcpName(n);
      ...
      function scan(session) {
        ...
        for (var i = 0; i < partial.length; i++) {
          var b = partial[i];
          if (b && b.kind === "tool-call" && typeof b.name === "string" && b.name.indexOf("mcp__") === 0) noteMcpName(b.name);
        }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("tool.call.toolview", function () {
        ...
        // MCP 工具名在运行期才知道：先按缓存注册，再随发现增量补注册（出厂卡片是 fallback，
        // 不占 key，所以这里不需要 SHADOW_PRIORITY 之外的额外协调）。
        disposers.push(claimMcpKeys((name) => ctx.slots.register(
          { name: "tool.call.toolview", key: name, priority: SHADOW_PRIORITY },
          (props) => e(ToolRow, props),
        )));
        disposers.push(startMcpDiscovery(ctx));
        return function () {
      ...
      ctx.slots.inject("conversation.chat.streamingCall", function () {
        ...
        disposers.push(claimMcpKeys((name) => ctx.slots.register(
          { name: "conversation.chat.streamingCall", key: name },
          (props) => e(StreamingToolRow, props),
        )));
        return function () {
          setSeatLive(false);
