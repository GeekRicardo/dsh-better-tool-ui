  ...
  factory: (require) => {
    ...
    function svgIcon(kind) {
      ...
      switch (kind) {
        ...
        case "chevron-left": return e("svg", common, PL("15 18 9 12 15 6"));
        case "chevron-right": return e("svg", common, PL("9 18 15 12 9 6"));
    ...
    // 纯勾（DSH IconCheckOutline14 同款，12×12），多选 checkbox 的选中态
    function askCheckIcon() {
      return e("svg", { width: 12, height: 12, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2.5, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true" }, e("polyline", { points: "20 6 9 17 4 12" }));
    }

    // 推荐后缀（(Recommended) / （推荐））拆成独立 badge，不改变答案匹配值
    function splitRecommended(label) {
      var m = /(?:\((?:recommended|推荐)\)|（(?:recommended|推荐)）)\s*$/i.exec(String(label || ""));
      if (!m) return { label: label, recommended: false };
      return { label: label.slice(0, m.index).trim(), recommended: true };
    }
    ...
    function AskPanel(props) {
      ...
      // header：eyebrow（短标题）+ title（问题正文）+ detail，层级对齐 DSH composer
      var head = [];
      if (typeof q.header === "string" && q.header) head.push(e("div", { className: "btu-ask-eyebrow", key: "e" }, q.header));
      head.push(e("div", { className: "btu-ask-title", key: "t" }, q.question));
      if (typeof q.detail === "string" && q.detail) head.push(e("div", { className: "btu-ask-detail", key: "d" }, q.detail));

      // 选项行：单选数字序号 / 多选 checkbox，选中态按 DSH optionSelected（背景 + 描边）
      var opts = Array.isArray(q.options) ? q.options : [];
      var rows = opts.map(function (opt, j) {
        ...
        var disp = splitRecommended(label);
        var ind = multi
          ? e("span", { className: "btu-ask-ind btu-ask-check", "data-on": isSel ? "1" : "0", key: "ind" }, isSel ? askCheckIcon() : null)
          : e("span", { className: "btu-ask-ind btu-ask-number", key: "ind" }, String(j + 1));
        var copy = [e("span", { className: "btu-ask-option-label", key: "l" }, disp.label)];
        if (disp.recommended) copy.push(e("span", { className: "btu-ask-badge", key: "b" }, "推荐"));
        if (opt && typeof opt.description === "string" && opt.description) copy.push(e("span", { className: "btu-ask-option-desc", key: "d" }, opt.description));
        return e("div", { className: "btu-ask-option", "data-sel": isSel ? "1" : "0", key: "o" + j },
          ind,
          e("span", { className: "btu-ask-option-copy", key: "copy" },
            e("span", { className: "btu-ask-option-line", key: "line" }, copy)));
      });

      // custom 自由文本：option 形状行，单选用编辑图标、多选复用 checkbox
      if (ans && typeof ans.custom === "string" && ans.custom !== "") {
        var cind = multi
          ? e("span", { className: "btu-ask-ind btu-ask-check", "data-on": "1", key: "ind" }, askCheckIcon())
          : e("span", { className: "btu-ask-ind btu-ask-custom-ind", key: "ind" }, svgIcon("pencil"));
        rows.push(e("div", { className: "btu-ask-custom", key: "c" },
          cind,
          e("span", { className: "btu-ask-custom-label", key: "v" }, ans.custom)));
      }

      var status = null;
      if (running) status = e("div", { className: "btu-ask-status", key: "st" }, "等待回答…");
      else if (!ans) status = e("div", { className: "btu-ask-status", key: "st" }, "未回答");

      var pager = null;
      if (count > 1) {
        pager = e("div", { className: "btu-ask-pager", key: "pager" },
          e("button", { type: "button", className: "btu-ask-icon-btn", key: "prev", "aria-label": "上一个问题", disabled: idx <= 0 ? true : undefined, onClick: function () { setPage(function (p) { return p <= 0 ? 0 : p - 1; }); } }, svgIcon("chevron-left")),
          e("span", { className: "btu-ask-progress", key: "label" }, (idx + 1) + " / " + count),
          e("button", { type: "button", className: "btu-ask-icon-btn", key: "next", "aria-label": "下一个问题", disabled: idx >= count - 1 ? true : undefined, onClick: function () { setPage(function (p) { return p >= count - 1 ? count - 1 : p + 1; }); } }, svgIcon("chevron-right")));
      }

      var footer = (pager || status)
        ? e("div", { className: "btu-ask-footer", key: "footer" }, pager, status)
        : null;

      return e("div", { className: "btu-ask", key: "ask" },
        e("div", { className: "btu-ask-head", key: "head" }, head),
        rows.length > 0 ? e("div", { className: "btu-ask-options", key: "opts" }, rows) : null,
        footer);
    }
...
/* ask_user_question：DSH composer 同款展示态（数字序号/checkbox/选项行/分页器对齐原生） */
.btu-ask { padding: 2px 0 0; }
.btu-ask-head { padding: 2px 2px 4px; }
.btu-ask-eyebrow { margin-bottom: 5px; color: var(--dsw-alias-label-tertiary); font-size: 11px; line-height: 16px; }
.btu-ask-title { font-size: 16px; line-height: 22px; font-weight: 500; color: var(--dsw-alias-label-primary); }
.btu-ask-detail { margin: 6px 2px 2px; color: var(--dsw-alias-label-tertiary); font-size: 13px; line-height: 20px; white-space: pre-wrap; word-break: break-word; }
.btu-ask-options { display: flex; flex-direction: column; gap: 1px; margin: 8px 0 0; }
.btu-ask-option { display: flex; align-items: flex-start; gap: 8px; min-height: 40px; padding: 8px 12px 8px 8px; border: 1px solid transparent; border-radius: 12px; }
.btu-ask-option[data-sel="1"] { background: var(--dsw-alias-interactive-bg-hover); border-color: var(--dsw-alias-border-l2); }
.btu-ask-ind { display: grid; place-items: center; flex: 0 0 20px; width: 20px; height: 20px; margin-top: 2px; }
.btu-ask-number { border-radius: 6px; background: var(--dsw-alias-bg-overlay); color: var(--dsw-alias-label-secondary); font-size: 12px; font-weight: 500; line-height: 18px; }
.btu-ask-check { display: grid; place-items: center; }
.btu-ask-check::before { content: ''; grid-area: 1 / 1; width: 14px; height: 14px; border: 1px solid var(--dsw-alias-border-l4); border-radius: 4px; }
.btu-ask-check > svg { grid-area: 1 / 1; }
.btu-ask-check[data-on="1"] { color: var(--dsw-alias-label-primary-foreground); }
.btu-ask-check[data-on="1"]::before { border-color: var(--dsw-alias-label-primary); background: var(--dsw-alias-label-primary); }
.btu-ask-option-copy { min-width: 0; flex: 1; }
.btu-ask-option-line { display: flex; align-items: baseline; flex-wrap: wrap; gap: 2px 6px; }
.btu-ask-option-label { font-size: 14px; line-height: 24px; font-weight: 500; color: var(--dsw-alias-label-primary); }
.btu-ask-option-desc { color: var(--dsw-alias-label-tertiary); font-size: 14px; line-height: 24px; font-weight: 400; }
.btu-ask-badge { padding: 0 4px; border-radius: 6px; background: var(--dsw-specific-sidebar-nav-item-active-accent); color: var(--dsw-alias-button-info-fill); font-size: 11px; line-height: 18px; font-weight: 600; }
.btu-ask-custom { display: flex; align-items: flex-start; gap: 8px; min-height: 40px; padding: 8px 12px 8px 8px; border: 1px solid transparent; border-radius: 12px; }
.btu-ask-custom-ind { color: var(--dsw-alias-label-secondary); }
.btu-ask-custom-label { font-size: 14px; line-height: 24px; color: var(--dsw-alias-label-primary); white-space: pre-wrap; word-break: break-word; }
.btu-ask-footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-top: 8px; }
.btu-ask-pager { display: flex; align-items: center; gap: 6px; }
.btu-ask-icon-btn { display: grid; place-items: center; width: 24px; height: 24px; padding: 0; border: none; border-radius: 999px; background: transparent; color: var(--dsw-alias-label-tertiary); cursor: pointer; }
.btu-ask-icon-btn:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover); color: var(--dsw-alias-label-primary); }
.btu-ask-icon-btn:disabled { color: var(--dsw-alias-label-dimmed); cursor: default; }
.btu-ask-progress { padding: 0 4px; color: var(--dsw-alias-label-secondary); font-size: 14px; line-height: 24px; font-weight: 500; white-space: nowrap; word-spacing: -2px; }
.btu-ask-status { color: var(--dsw-alias-label-tertiary); font-size: 12px; line-height: 16px; }
